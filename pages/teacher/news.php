                ...
                <div class="manage-student_wraper">
                    ...
                    <form method="POST" action="<?= BASE_URL; ?>pages/teacher/news.php">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control" id="newsTitle" value="<?= htmlspecialchars($title); ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="newsContent" class="form-label">News Content</label>
                            <textarea name="content" class="form-control" id="newsContent" rows="3" required><?= htmlspecialchars($content); ?></textarea>
                        </div>
                        <?php if (isset($_GET['action']) && $_GET['action'] === 'edit'): ?>
                            <input type="hidden" name="news_id" value="<?= htmlspecialchars($_GET['id']); ?>">
                        <?php endif; ?>
